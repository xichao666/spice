...
typedef struct { char name[NAME_SIZE]; double is, af, ar; } NpnModel;
typedef struct { int c, b, e, model; char model_name[NAME_SIZE]; } Npn;
/* 已解析的电路数据与温度设置。 */
typedef struct {
    int node_ids[DC_MAX_UNKNOWNS], node_count;
    Resistor r[MAX_ELEMENTS]; int r_count;
    VoltageSource v[MAX_ELEMENTS]; int v_count;
    Npn q[MAX_ELEMENTS]; int q_count;
    NpnModel model[MAX_MODELS]; int model_count;
...
static bool parse_model(NetlistCircuit *circuit, char *line)
{
    ...
    if (count < 3 || _stricmp(token[0], ".model") != 0 ||
        _stricmp(token[2], "NPN") != 0 || circuit->model_count >= MAX_MODELS)
        return false;
    NpnModel *model = &circuit->model[circuit->model_count++];
    snprintf(model->name, sizeof(model->name), "%s", token[1]);
    model->is = 1e-14; model->af = 100.0 / 101.0; model->ar = 0.5;
    for (int i = 3; i + 1 < count; i += 2) {
        char key[NAME_SIZE]; snprintf(key, sizeof(key), "%s", token[i]); upper_string(key);
...
static bool parse_netlist(const char *path, NetlistCircuit *circuit)
{
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        char name[NAME_SIZE], a[NAME_SIZE], b[NAME_SIZE], c[NAME_SIZE], d[NAME_SIZE];
        const int fields = sscanf(cursor, "%31s %31s %31s %31s %31s", name, a, b, c, d);
...
{
    const NpnModel *m = &circuit->model[model_index];
                         ...
                         double *f, double j[DC_MAX_UNKNOWNS][DC_MAX_UNKNOWNS])
{
    ...
    for (int i = 0; i < circuit->q_count; ++i) {
        const Npn *q = &circuit->q[i]; const BjtParameters p = bjt_parameters(circuit, q->model);
        const double vc = node_voltage(x, q->c), vb = node_voltage(x, q->b), ve = node_voltage(x, q->e);
        const double ef = safe_exp((vb - ve) / p.vt), er = safe_exp((vb - vc) / p.vt);
        const double ifc = p.is * (ef - 1.0), irc = p.is * (er - 1.0), gf = p.is * ef / p.vt, gr = p.is * er / p.vt;
        const double ic = ifc - irc / p.ar;
        const double ib = (1.0 / p.af - 1.0) * ifc + (1.0 / p.ar - 1.0) * irc;
        const double ie = ifc / p.af - irc;
...
{
    const char *path; bool direct_nr = false; NetlistCircuit circuit; DcProblem problem;
    DcSolverOptions options = dc_solver_default_options(); double x[DC_MAX_UNKNOWNS] = {0.0}; int iterations = 0;
    if (argc < 2) { fprintf(stderr, "Usage: %s netlist.txt [--nr] [--temp Celsius]\n", argv[0]); return 1; }
    path = argv[1];
    for (int i = 2; i < argc; ++i) {
        if (strcmp(argv[i], "--nr") == 0) direct_nr = true;
    }
    if (!parse_netlist(path, &circuit)) { fprintf(stderr, "Could not parse supported R/V/Q NPN netlist: %s\n", path); return 1; }
    for (int i = 2; i + 1 < argc; ++i)
        if (strcmp(argv[i], "--temp") == 0) circuit.temperature_celsius = atof(argv[++i]);
    problem = (DcProblem) { circuit.node_count + circuit.v_count, &circuit, residual, jacobian };
    if (direct_nr) {
        if (!dc_newton_solve(&problem,&options,1.0,x,&iterations)) { fputs("Newton did not converge.\n",stderr); return 1; }
        printf("Direct Newton converged in %d iterations.\n", iterations);
    } else {
        int total = 0;
